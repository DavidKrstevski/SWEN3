...
namespace Paperless_API.Messaging
{
    public class RabbitMqConsumer : IDisposable
    {
        ...
        public RabbitMqConsumer(IConfiguration config, ILogger<RabbitMqConsumer> logger)
        {
            ...
            _queueName = config["RABBITMQ_OCR_COMPLETED_QUEUE"] ?? "ocr_completed_legal";
        ...
        public async Task ConsumeAsync(Func<Document, Task> handler, CancellationToken ct)
        {
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                ...
                try
                {
                    var msg = JsonSerializer.Deserialize<Document?>(
                        json,
